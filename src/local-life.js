...
export async function loadLocalLife(city,{signal,onUpdate}){
 ...
 const jobs=[['rents',async()=>({values:rentalProfile(await cached(import.meta.env.BASE_URL+'rents-2025/'+city.code.slice(0,2)+'.json',signal),city.code)})],['schools',async()=>{const raw=await cached(schools,signal);if(!Array.isArray(raw.results))throw Error();return {rows:raw.results,source:schools};}],['water',()=>cached('/api/water?code='+city.code,signal)],['transport',()=>loadTransport(city.code,signal)],['services',async()=>({...publicServicesProfile(await cached(directory,signal)),source:directory})],['energy',async()=>({...energyProfile(await cached(energy,signal)),source:energy})]];
